...
object Main extends App{
   ...
   def processFile(file_path: File): FileProcessingParams = {
       try {
           ...
           println("sensor_map ", sensor_map)
           println("no_of_records no_of_failed_records", no_of_records, no_of_failed_records)
           source_file.close()
   ...
   def merge_sensor_maps(overall_map: Map[String, ArrayBuffer[String]], current_map: Map[String, ArrayBuffer[String]]):
       ...
       for(element <- current_map.keys) {
           ...
           println("final map in merge_sensor_maps-> ", final_map)
       }
       final_map
   }
           ...
           for (x <- arrayBuffer) {
               breakable {
                   ...
                   if (x.toInt < min)
                       min = x.toInt
                   sum = sum + x.toInt
                   println("",x, min,sum, max)
               }
           }
           ArrayBuffer(min.toString, (sum/arrayBuffer.size).toString, max.toString)
       }
   }
}
